...
import { generateJsonWithGemini } from "@/lib/gemini-ai"
...
export async function findMatchesWithGemini(userProfile: UserProfile): Promise<MatchResult[]> {
  try {
...
`
    ...
    // Call AI using our utility
    const response = await generateJsonWithGemini<MatchData[]>(prompt, {
      temperature: 0.2,
      maxTokens: 2000,
    })

    if (!response.success || !response.data) {
      console.error("AI matchmaking failed:", response.error)
