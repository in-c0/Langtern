...
import { translateTextWithGemini, detectLanguageWithGemini } from "@/actions/gemini-translation"
...
export function useGeminiTranslation(initialSettings?: Partial<TranslationSettings>) {
  ...
  const translateMessage = useCallback(
    async (text: string, targetLanguage?: string) => {
      ...
      try {
        ...
        const result = await translateTextWithGemini(text, target, source)
        return result
      } catch (error) {
